...
then
	...
	while [ $WHOLE_REMAINING != 0 ] && [ $WHOLE_REMAINING != 1 ]; do
		...
		if [ $WHOLE_REMAINING != 0 ] && [ $WHOLE_REMAINING != 1 ]; then
	...
	while [ $USE_LVM != 0 ] && [ $USE_LVM != 1 ]; do
		...
		(0) Use standard partitioning
		(1) Use LVM"
		read -p "> " USE_LVM
		if [ $USE_LVM != 0 ] && [ $USE_LVM != 1 ]; then
			echo " Invalid input: You must provide either \"0\" or \"1\""
		fi
	done
	...
	while [ $USE_ENCRYPTION != 0 ] && [ $USE_ENCRYPTION != 1 ]; do
		...
		(0) DON'T encrypt partitions
		(1) Encrypt partitions"
		read -p "> " USE_ENCRYPTION
		if [ $USE_ENCRYPTION != 0 ] && [ $USE_ENCRYPTION != 1 ]; then
			echo " Invalid input: You must provide either \"0\" or \"1\""
		fi
	done
